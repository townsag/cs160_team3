...
// Calculate the total price
    ...
    // Set the price variable to the total price
    let price = 0;
    ...
    async function handlePaymentSubmit() {
        storedCardNumber = cardNumberState;
        storedCardExpiry = cardExpiryState;
        storedCardCvc = cardCvcState;

        console.log(storedCardNumber);
        console.log(storedCardExpiry);
        console.log(storedCardCvc);

        const result = await makePayment(storedCardNumber, storedCardExpiry, storedCardCvc);
        ...
        if (result.success) {
            ...
            paymentErrorTextState = result.error || 'Payment failed.';
        }
    }
</script>
    ...
    <div class="flex">
        ...
        <div class="card w-full md:w-2/5 xl:w-2/5 bg-base-100 border-2 border-black-500 mt-8 ">
            ...
            <div class="card-body pl-20 pr-20 pt-10">
                ...
                <h1 class="card-title">Cart:<p style="text-align: right;">{price}</p> </h1>
                <h1 class="card-title">Tax:<p style="text-align: right;">{price * 0.05}</p> </h1>
                <h1 class="card-title">Shipping:<p style="text-align: right;">{price * 0.1}</p> </h1>
                <h1 class="card-title">Total:<p style="text-align: right;">{(price * 0.05) + (price * 0.5) + price}</p> </h1>
